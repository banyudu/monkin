...
final class PetView: NSView {
    ...
    private var activeProgram = MotionPresets.idle
    ...
    func setMotionStyle(_ style: String) {
        ...
        activeProgram = MotionPresets.program(for: validatedStyle)
#if canImport(RiveRuntime)
    ...
    func setMotionProgram(_ program: MotionProgram) {
        activeProgram = MotionInterpreter.validate(program).program
    }
    ...
    private func advanceMotion() {
        motionTime += 1.0 / 12.0
        let nextPose = MotionInterpreter.pose(for: activeProgram, at: motionTime)

#if canImport(RiveRuntime)
        if let riveRenderer, !riveRenderer.isHidden {
            riveRenderer.applyPose(nextPose)
            return
        }
#endif
